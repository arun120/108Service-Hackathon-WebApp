...
<body>
 <%@ page import="java.sql.*" %>  
  <%@page import="Database.Dbdetails"%>
        ...
        String a = "",e="",query = "SELECT a.DRIVER_ID,a.LATITUDE,a.LONGITUDE,b.NAME,b.TYPE,b.MOBILE FROM DRIVER_DETAILS b,DRIVER_LOCATION a WHERE a.DRIVER_ID=b.DRIVER_ID ";
...
%>
         ...
         myCenter = new google.maps.LatLng("<%=rs.getString("latitude")%>","<%=rs.getString("longitude")%>");
              marker<%=i%> = new google.maps.Marker({position:myCenter});
             marker<%=i%>.setMap(map);
           
             infowindow<%=i%> = new google.maps.InfoWindow({
            content :"<br>Driver_id "+<%=rs.getString("driver_id")%>+"<br>"+"Name:"+"<%=rs.getString("name")%>"
            });
...
<%
      ...
      <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDmpIyxCnOqXMfcl4mi-iZub0XQlgjkTgY&callback=myMap"></script>
            ...
            <div class="span3">
              <ul class="site-stats">
                <% 
                    ...
                    String query1 ="SELECT COUNT(*) AS COUNT  FROM CUSTOMER_DETAILS";
                     rs = stmt.executeQuery(query1);
                     rs.next();
                %>
                <li class="bg_lh"><i class="icon-user"></i> <strong><%=rs.getString("count") %></strong> <small>Total App Users</small></li>
